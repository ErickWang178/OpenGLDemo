...
int main(){
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//设置材质属性
		GLint loc;
		loc = glGetUniformLocation(myShader.mProgram, "material.ambient");
		glUniform3f(loc, 1.0f, 0.5f, 0.31f);

		loc = glGetUniformLocation(myShader.mProgram, "material.diffuse");
		glUniform3f(loc, 1.0f, 0.5f, 0.31f);

		loc = glGetUniformLocation(myShader.mProgram, "material.specular");
		glUniform3f(loc, 0.5f, 0.5f, 0.5f);

		loc = glGetUniformLocation(myShader.mProgram, "material.shininess");
		...
		loc = glGetUniformLocation(myShader.mProgram, "light.ambient");
		glUniform3f(loc, ambientColor.x,ambientColor.y,ambientColor.z);
		loc = glGetUniformLocation(myShader.mProgram, "light.diffuse");
		glUniform3f(loc,diffuseColor.x,diffuseColor.y,diffuseColor.z);
		loc = glGetUniformLocation(myShader.mProgram, "light.specular");
		...
		viewMat = glm::translate(viewMat, glm::vec3(0.0f, -0.5f, -5.0f));
...
// 		
		...
		//绘制光源
		lightShader.use();
		glm::vec3 lightPos(1.0f, 1.0f, 6.0f);
